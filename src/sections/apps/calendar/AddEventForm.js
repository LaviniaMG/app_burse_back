...
const AddEventFrom = ({ event, range, onCancel, modalCallback }) => {
  ...
  const EventSchema = Yup.object().shape({
    title: Yup.string().max(255).required('Titlul este obligatoiu!'),
    description: Yup.string().max(5000),
    end: Yup.date().when('start', (start, schema) => start && schema.min(start, 'Data finala trebuie sa fie dupa data de start!')),
    start: Yup.date(),
  ...
  const deleteHandler = async () => {
    ...
    openSnackbar({
      open: true,
      message: 'Eveniment sters cu succes!',
      variant: 'alert',
      alert: {
        color: 'success'
      }
    ...
    onSubmit: async (values, { setSubmitting }) => {
      try {
        ...
        if (event) {
          ...
          openSnackbar({
            open: true,
            message: 'Eveniment adaugat cu succes!',
            variant: 'alert',
            alert: {
              color: 'success'
            }
    ...
    <FormikProvider value={formik}>
      <LocalizationProvider dateAdapter={AdapterDateFns}>
        <Form autoComplete="off" noValidate onSubmit={handleSubmit}>
          <DialogTitle>{event ? 'Modificare eveniment' : 'Adaugare eveniment'}</DialogTitle>
          <Divider />
          <DialogContent sx={{ p: 2.5 }}>
            <Grid container spacing={3}>
              <Grid item xs={12}>
                <Stack spacing={1}>
                  <InputLabel htmlFor="cal-title">Titlu</InputLabel>
                  <TextField
                    fullWidth
                    id="cal-title"
                    placeholder="Titlu"
              ...
              <Grid item xs={12}>
                <Stack spacing={1}>
                  <InputLabel htmlFor="cal-description">Descriere</InputLabel>
                  <TextField
                    fullWidth
                    id="cal-description"
                    multiline
              ...
              <Grid item xs={12}>
                <FormControlLabel control={<Switch checked={values.allDay} {...getFieldProps('allDay')} />} label="Toată ziua" />
              </Grid>
              <Grid item xs={12} md={6}>
                <Stack spacing={1}>
                  <InputLabel htmlFor="cal-start-date">Dată de inceput</InputLabel>
              ...
              <Grid item xs={12} md={6}>
                <Stack spacing={1}>
                  <InputLabel htmlFor="cal-end-date">Dată de final</InputLabel>
                  <MobileDateTimePicker
              ...
              <Grid item xs={12}>
                <Grid container spacing={2}>
                  <Grid item xs={12}>
                    <Typography variant="subtitle1">Culoare de fundal</Typography>
                  </Grid>
                  <Grid item xs={12}>
                    <FormControl>
                      <RadioGroup
              ...
              <Grid item xs={12}>
                <Grid container spacing={2}>
                  <Grid item xs={12}>
                    <Typography variant="subtitle1">Culoare pentru text</Typography>
                  </Grid>
                  <Grid item xs={12}>
          ...
          <DialogActions sx={{ p: 2.5 }}>
            <Grid container justifyContent="space-between" alignItems="center">
              <Grid item>
                {!isCreating && (
                  <Tooltip title="Ștergere eveniment" placement="top">
