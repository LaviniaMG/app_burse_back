...
const BursaStudiu = () => {
  ...
  const updateScholarshipQuantities = async () => {
    ...
    try {
      const response = await fetch(`http://localhost:8080/api/tipBursa/update/${3}`, {
        method: 'PUT',
        headers: {
  ...
  const calculateNrBursieri = async () => {
    ...
    try {
      const response = await fetch(`http://localhost:8080/uploads-students/${3}`, {
        method: 'POST',
        body: formData,
      });
