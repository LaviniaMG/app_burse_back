...
const BursaSocialaPrivata = () => {
  ...
  useEffect(() => {
    const fetchBuget = async () => {
      try {
        const response = await fetch('http://localhost:8080/api/tipBursa/11'); // Presupunem că id-ul bursăi este 11
  ...
  useEffect(() => {
    const fetchDataFinal = async () => {
      try {
        const response = await fetch('http://localhost:8080/api/tipBursa/11');
        const data = await response.json();
  ...
  const handleIdentifyStudents = async () => {
    ...
    try {
      const response = await fetch('http://localhost:8080/api/concurs/mihailManoilescu', {
        method: 'GET',
        headers: {
          'Content-Type': 'application/json'
        }
...
export default  BursaSocialaPrivata;
