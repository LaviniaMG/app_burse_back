...
const BursaSpeciala = () => {
  ...
  useEffect(() => {
    const fetchBuget = async () => {
      try {
        const response = await fetch('http://localhost:8080/api/tipBursa/10'); // Presupunem că id-ul bursăi este 10
  ...
  useEffect(() => {
    const fetchDataFinal = async () => {
      try {
        const response = await fetch('http://localhost:8080/api/tipBursa/10');
        const data = await response.json();
  ...
  const handleIdentifyStudents = async () => {
    ...
    try {
      const response = await fetch('http://localhost:8080/api/concurs/rezultateDeosebite', {
        method: 'GET',
        headers: {
          'Content-Type': 'application/json'
        }
