...
const BursaCercetare = () => {
  ...
  useEffect(() => {
    const fetchBuget = async () => {
      try {
        const response = await fetch('http://localhost:8080/api/tipBursa/9'); // Presupunem că id-ul bursăi este 9
  ...
  useEffect(() => {
    const fetchDataFinal = async () => {
      try {
        const response = await fetch('http://localhost:8080/api/tipBursa/9');
        const data = await response.json();
  ...
  const handleIdentifyStudents = async () => {
    ...
    try {
      const response = await fetch('http://localhost:8080/api/concurs/performanta', {
        method: 'GET',
        headers: {
          'Content-Type': 'application/json'
        }
