...
const BursaSociala = () => {
  ...
  useEffect(() => {
    const fetchBuget = async () => {
      try {
        const response = await fetch('http://localhost:8080/api/tipBursa/6'); // Presupunem că id-ul bursăi este 4
  ...
  useEffect(() => {
    const fetchDataFinal = async () => {
      try {
        const response = await fetch('http://localhost:8080/api/tipBursa/6');
        const data = await response.json();
