...
const BursaExOlimpica = () => {
  ...
  useEffect(() => {
    const fetchBuget = async () => {
      try {
        const response = await fetch('http://localhost:8080/api/tipBursa/8'); // Presupunem că id-ul bursăi este 8
  ...
  useEffect(() => {
    const fetchDataFinal = async () => {
      try {
        const response = await fetch('http://localhost:8080/api/tipBursa/8');
        const data = await response.json();
  ...
  const handleIdentifyStudents = async () => {
    ...
    try {
      const response = await fetch('http://localhost:8080/api/concurs/remarcabile', {
        method: 'GET',
        headers: {
          'Content-Type': 'application/json'
        }
...
export default BursaExOlimpica;
