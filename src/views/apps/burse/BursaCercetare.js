...
const BursaCercetare = () => {
  ...
  useEffect(() => {
    const fetchBuget = async () => {
      try {
        const response = await fetch('http://localhost:8080/api/tipBursa/4'); // Presupunem că id-ul bursăi este 4
  ...
  useEffect(() => {
    const fetchDataFinal = async () => {
      try {
        const response = await fetch('http://localhost:8080/api/tipBursa/4');
        const data = await response.json();
  ...
  const handleIdentifyStudents = async () => {
    ...
    try {
      const response = await fetch('http://localhost:8080/api/concurs/cercetrareStiintifica', {
        method: 'GET',
        headers: {
          'Content-Type': 'application/json'
        }
